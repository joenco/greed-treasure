package TablaCaballero;
	...
	public class ArmaCaballero {
		...
		private TestTableModel2 tableDtaModel;
		...
		private void initGUI() {
			...
			tableDtaModel = new TestTableModel2();
		...
		private TableColModel initTableColModel() {
			...
			tableColumn = new TableColumn() {
				@Override
				public Object getValue(ETable table, Object element) {
					ArmaBean2 armasBean = (ArmaBean2) element;

					// return new
					// ResourceImageReference(personaBean.getImageName());
					System.err.println(armasBean.getImageName());
					return ImageReferenceCache2.getInstance().getImageReference(
	...
	//Las columnas 
		...
		private NestedCellRenderer initNestedCellRenderer() {
			...
			nestedCellRenderer.getCellRendererList().add(new BaseCellRenderer() {
				...
				public Component getCellRenderer(final ETable table,
					...
					boolean editable = ((TestTableModel2) table.getTableDtaModel())
							.getEditable();

					Button ret = new Button("USAR");
					ret.setStyle(GUIStyles2.DEFAULT_STYLE);
		...
		private void btnEdtClicked(ETable table, int row) {
			...
			ArmaBean2 ArmaBean = (ArmaBean2) model.getElementAt(row);
		...
		private void btnDelClicked(ETable table, int row) {
			...
			TableDtaModel model = table.getTableDtaModel();
			ArmaBean2 armaBean = (ArmaBean2) model.getElementAt(row);
			inputWindow2 inputWindow = new inputWindow2(armaBean);
			add(inputWindow);

		}

		private void add(Object inputWindow2) {
			// TODO Auto-generated method stub
		...
		private void btnCreateClicked() {
			ArmaBean2 armaBean = new ArmaBean2();
		...
		private void btnUpdateClicked() {
			ArmaBean2 armaBean = (ArmaBean2) tableDtaModel.getElementAt(editingRow);
			armaBean.setFrstName(txtFrst.getText());
			armaBean.setLastName(txtLast.getText());
		...
		public void crear_tabla() {
			 List<ArmaBean2> armasBeanList = ArmaBeanLoader2.loadData();
			 for (ArmaBean2 armasBean : armasBeanList) {
